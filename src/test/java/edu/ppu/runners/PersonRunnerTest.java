...
public class PersonRunnerTest {
	...
	@Test
	public void testGetOldestGuy() {
		PersonService mockPersonServiceImpl = mock(PersonService.class);
		when(mockPersonServiceImpl.getPersons(anyInt()))
				.thenReturn(Arrays.asList(new Person("child", 10), new Person("old", 70), new Person("new", 80)));
		PersonRunner cut = new PersonRunner(mockPersonServiceImpl);
		Person person = cut.getOldestGuy();

		Person maxAge = new Person("new", 80);
		
		assertEquals("invaild age", maxAge,  person);
	}
}
